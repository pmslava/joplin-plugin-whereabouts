...
/**
 * Launch a real Joplin desktop (Electron) instance with the Whereabouts plugin loaded as a
 * development plugin, against a throwaway profile, and drive it with Playwright over CDP.
 *
 * Adapted from joplin-plugin-ridgeline's harness. Why CDP instead of Playwright's `_electron.launch`:
...
export interface JoplinInstance {
  ...
  port: number;
  /**
   * The Data API token seeded into this profile. Specs use it to create notebooks and notes over
   * Joplin's own REST API instead of clicking through the GUI — the only practical way to build a
   * NESTED notebook (the sidebar's only path to one is a native context menu Playwright cannot
   * drive). The token is random per profile, which is also what makes port discovery safe: a
   * different Joplin already listening on 41184 rejects it. See e2e/dataApi.ts.
   */
  apiToken: string;
}
...
export function createProfile(loadPlugin = true, seed: SeedSettings = {}): string {
  ...
  // Whereabouts settings are registered with SettingStorage.File, so keys written here under the
  // plugin namespace are loaded by Joplin at startup.
  const prefix = `plugin-${PLUGIN_ID}.`;
  if (seed.pathMode) settings[`${prefix}pathMode`] = seed.pathMode;
  if (seed.placement) settings[`${prefix}placement`] = seed.placement;
  if (seed.hideNativePill != null) settings[`${prefix}hideNativePill`] = seed.hideNativePill;
  if (seed.separator != null) settings[`${prefix}separator`] = seed.separator;
  if (seed.showIcon != null) settings[`${prefix}showIcon`] = seed.showIcon;
...
export async function launchJoplin(
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      return await startInstance(profileDir, apiToken);
    } catch (error) {
      lastError = error;
...
/** Read back the Data API token createProfile seeded, so specs can talk to this instance only. */
function readApiToken(profileDir: string): string {
  const raw = JSON.parse(fs.readFileSync(path.join(profileDir, 'settings.json'), 'utf8'));
  const token = raw['api.token'];
  if (typeof token !== 'string' || !token) throw new Error(`No api.token in ${profileDir}/settings.json`);
  return token;
}

async function startInstance(profileDir: string, apiToken: string): Promise<JoplinInstance> {
  ...
  try {
    ...
    return { browser, child, win, profileDir, port, apiToken };
  } catch (error) {
    try {
      child.kill('SIGKILL');
    } catch {
